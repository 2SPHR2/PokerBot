...
async function createNewThreadForUser(user, quiet = false, ignoreRequirements = false) {
  ...
  let createdChannel;
  try {
    createdChannel = await utils.getInboxGuild().createChannel(channelName, null, ':heavy_plus_sign:**Ticket Jadid!** **|** **Creator:**', newThreadCategoryId);
  } catch (err) {
  ...
  if (! quiet) {
    ...
    if (config.mentionRole) {
      await newThread.postNonLogMessage({
        content: `${utils.getInboxMention()}:heavy_plus_sign:**Ticket Jadid!** **|** **Creator:** ${newThread.user_name}`,
  ...
  if (userLogCount > 0) {
    infoHeader += `\n\nTedad Ticket haye In Kaarbar:** ${userLogCount} |** Didan Ticket Haaye Ghablie In Kaarbar: \`${config.prefix}logs\` `;
  }
